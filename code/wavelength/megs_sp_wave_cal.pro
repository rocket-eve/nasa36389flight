...
function megs_sp_wave_cal, spectrum, err, status, file=file, param=param, megsb=megsb,debug=debug, slit1=slit1, order=order

common	megs_sp_wave_cal, megsb_wave_set, ref_wave, cal_wave, last_file
...
;    param = [ 31.679, dwave ]
    ...
    ; for 36.318, try 31.75 instead of 32.425
    dwave = (97.702-36.8) / (1811.-140.)
    pre_wave = 31.75 + dwave * findgen(NUM_MEGSB_COLS) ;initial guess
    param = [ 31.75, dwave ]
...
;
   ...
   new_coeff = poly_fit(CM_anodes[pick], cal_wave[pick].wave, order, Yfit, $
                   Yband, fit_err )
